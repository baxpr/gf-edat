...
if r is None:
    print(f'   WARNING: Could not parse filename {eprime_txt}. Skipping')
    sys.exit(0)
...
with dax.XnatUtils.get_interface() as xnat:
    ...
    match = [x for x in scans if x['scan_type'].startswith(scan_prefix) and x['quality']!='unusable']
    
    # Only proceed if exactly one match is found
	if len(match) == 0:
        print(f'   WARNING: No matching scan for {eprime_txt}')
        sys.exit(0)
    elif len(match) > 1:
        print(f'   WARNING: Multiple matching scans found ({len(match)})')
        sys.exit(0)
    else:
        match = match[0]
